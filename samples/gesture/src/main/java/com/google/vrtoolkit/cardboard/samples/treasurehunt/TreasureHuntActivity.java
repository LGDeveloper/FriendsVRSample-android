...
public class TreasureHuntActivity
    ...
    // Start of Gesture Events
    private GestureDetector mGestureDetector;
    private int prevTouchPoints = 0;
    private final Handler mTouchHandler = new Handler();
    private final Runnable mTouchRunnable = new Runnable() {
        @Override
        public void run() {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mGestureDetector != null) {
            ...
            if (prevTouchPoints != points) {
                prevTouchPoints = points;
                mTouchHandler.removeCallbacks(mTouchRunnable);
                if (prevTouchPoints == 2) {
                    mTouchHandler.postDelayed(mTouchRunnable, 1000);
                }
            }
